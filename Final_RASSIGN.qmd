...
Czech_Republic <- fread("indicators_cze.csv", skip = 1, 
...
Complete_Data <- rbindlist(list(Bahrain, Czech_Republic, Algeria), 
                           use.names = TRUE, fill = TRUE)
```

Prose: The three country datasets are combined using rbindlist() function to create one unified Complete_Data table to ensures the consistency in column names and handling of the missing columns
...
skim_summary <- skim(Complete_Data)
```
...
StructureSummary_Result <- capture.output(
  str(Complete_Data, give.attr = FALSE, vec.len = 1))
...
setkey(Complete_Data, `#country+name`, `#date+year`)

# The_Focus_indicator: "Access to electricity"
Electricity_Data <- 
Complete_Data[`#indicator+name` == "Access to electricity, rural (% of rural population)"]
...
Cereal_Data <- 
  Complete_Data[`#indicator+name` == "Cereal yield (kg per hectare)"]
